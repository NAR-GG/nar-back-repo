...
public class CommunityWriteGuard {
	...
	/**
	 * 게시판 쓰기 자격. 전체 게시판(boardTeamId == null)은 로그인만 있으면 되고,
	 * 팀 게시판은 응원팀 일치 + 팀 변경 30일 쿨다운(D-1)을 본다. 글·댓글 공통.
	 */
	public void checkBoardWritable(Member member, Long boardTeamId) {
		...
		if (changedAt != null) {
			...
			long remaining = remainingSeconds(writableFrom);
			if (remaining > 0) {
				throw new CommunityWriteBlockedException(ErrorCode.COMMUNITY_TEAM_COOLDOWN, remaining);
			}
		}
	}
