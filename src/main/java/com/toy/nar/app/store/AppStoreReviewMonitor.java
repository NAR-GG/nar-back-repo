...
public class AppStoreReviewMonitor {
	...
	public void pollReviews() {
		...
		boolean seeding = repository.hasNoReviews(StoreNotificationRepository.PLATFORM_IOS);
		int notified = 0;
		// 응답은 최신순이다. 뒤에서부터 돌려 오래된 리뷰가 채널에 먼저 오게 한다.
		for (int i = reviews.size() - 1; i >= 0; i--) {
			CustomerReview review = reviews.get(i);
			boolean isNew = repository.markReviewSeen(
					StoreNotificationRepository.PLATFORM_IOS, review.id(), review.rating(), review.territory());
			if (!isNew || seeding) {
				continue;
			}
	...
	static String reviewTitle(CustomerReview review) {
		return String.format("[앱스토어 리뷰] %s %d점",
				StoreNotifyText.stars(review.rating()), review.rating());
	}

	static String reviewMessage(CustomerReview review) {
		return String.format("**%s**\n```text\n%s\n```\n작성자 `%s` · 지역 `%s` · 작성 `%s`",
				StoreNotifyText.orPlaceholder(review.title(), "(제목 없음)"),
				StoreNotifyText.orPlaceholder(review.body(), "(본문 없음)"),
				review.nickname(),
				review.territory(),
				StoreNotifyText.toSeoul(review.createdDate()));
	}
}
