...
public class MatchDetailFinder {
	...
	private MatchDetailResponseDto convertToMatchDetailDto(List<GameParticipant> participants, String matchId) {
		...
		List<MatchDetailResponseDto.GameDetailDto> gameDetails = participantsByGame.values().stream()
			.map(gameParticipants -> {
				...
				MatchDetailResponseDto.GameDetailDto.TeamPicksDto blueTeam = createTeamPicksDto(participantsBySide.get("Blue"));
				MatchDetailResponseDto.GameDetailDto.TeamPicksDto redTeam = createTeamPicksDto(participantsBySide.get("Red"));

				return new MatchDetailResponseDto.GameDetailDto(game.getId(), game.getGameNumber(), game.getGameLengthSeconds(), blueTeam, redTeam);
			})
	...
	private MatchDetailResponseDto.GameDetailDto.TeamPicksDto createTeamPicksDto(List<GameParticipant> teamParticipants) {
		...
		List<MatchDetailResponseDto.GameDetailDto.PlayerPickDto> players = teamParticipants.stream()
			.map(p -> new MatchDetailResponseDto.GameDetailDto.PlayerPickDto(p.getPosition(), p.getPlayer().getName(), p.getChampion().getChampionNameEn()))
