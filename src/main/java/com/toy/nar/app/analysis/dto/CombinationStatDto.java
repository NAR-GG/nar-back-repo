...
public class CombinationStatDto {
	...
	public CombinationStatDto(String championCombination, long frequency, long winCount, LocalDateTime latestGameDate,
			String patchVersions) {
		this.champions = List.of(championCombination.split(",")).stream()
				.map(String::trim)
				.filter(s -> !s.isEmpty())
				.toList();
