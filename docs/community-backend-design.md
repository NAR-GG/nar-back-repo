...
> **방향 전환 기록 (2026-08-30)** — 이 문서의 팀별 게시판(전체·응원팀·상대팀)
> 구조는 팀 피드백으로 **보류**됐다. 사유: 팀을 쪼개면 게시판마다 글이 안 쌓이고,
> 글이 없으니 사람이 안 오는 **글 리젠 악순환** — 지금 규모(회원 6.5천)에서는
> 하나의 전체 커뮤니티로 시작해 사용 패턴을 보고 되살릴지 정한다.
> 서버는 걷어내지 않았다: `board_team_id`·NOT_FAN 검사·팀 쿨다운은 **dormant**
> (앱 1.0.22+45 부터 `boardTeamId=null` 만 보낸다). 팀변경 30일 제한(#493)은
> 프로필 수정에서 유지. 알림함은 커뮤니티 전용으로 축소(`group=COMMUNITY`,
> #508), 경기 알림 진입점은 마이구독으로 복원. 다음 단계는 작성 툴 고도화
> (블록 본문 + 링크 프리뷰 + 임베드) — `docs/community-api-handoff.md` 참고.
