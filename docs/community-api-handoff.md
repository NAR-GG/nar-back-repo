...
### 테스트 글 — `test: true` (테스터 전용)

prod 에서 기능을 확인하되 일반 사용자 목록을 어지럽히지 않으려는 장치.

- 작성 요청에 `"test": true` → 글이 `status=TEST` 로 저장되고 **테스터에게만** 보인다
  (목록·상세·내 활동 모두. 딥링크로도 남에게 안 보인다)
- 테스터가 아닌 계정이 보내면 **조용히 무시**된다(일반 사용자가 숨은 글을 만들 수 없다)
- 테스터 여부는 목록 응답 `boardViewer.tester`(boolean)로 온다 — 앱은 이 값으로
  작성 화면의 "테스트 글" 토글 노출을 결정한다
- 테스터 지정은 서버 env `COMMUNITY_TESTER_MEMBER_IDS`(회원 id 쉼표 구분). 비우면 아무도 아님
