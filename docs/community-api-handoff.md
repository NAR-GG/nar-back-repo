...
1. **쿨다운 잠금 바를 넣을지 말지.** 잠금 바로 미리 보여주려면 앱이 게시판 진입
   시점에 쿨다운 상태를 알아야 하는데, 지금 서버는 쓰기 시도(403)에서만 알려준다.
   - 넣기로 하면: `/api/auth/me` 에 `teamBoardWritableFrom`(datetime, null=제한 없음)
     필드를 추가한다 — 서버 작업 얼마 안 됨. 앱은 이 값 하나로 잠금 바 문구,
